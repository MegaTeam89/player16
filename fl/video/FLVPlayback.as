   ...
   public class FLVPlayback extends Sprite
   {
      ...
      public function pause() : void
      {
         ...
         if(!flvplayback_internal::_firstStreamShown)
         {
            ...
            queueCmd(_loc1_,QueuedCommand.PAUSE);
         }
         else
         {
      ...
      public function stop() : void
      {
         ...
         {
            _loc1_ = flvplayback_internal::videoPlayerStates[_activeVP];
            queueCmd(_loc1_,QueuedCommand.STOP);
         }
         else
         {
            _loc2_ = flvplayback_internal::videoPlayers[_activeVP];
      ...
      override public function set soundTransform(param1:SoundTransform) : void
      {
         ...
         _soundTransform.volume = !!scrubbing ? Number(0) : Number(param1.volume);
      ...
      public function seek(param1:Number) : void
      {
         ...
         if(!flvplayback_internal::_firstStreamShown)
         {
            ...
            queueCmd(_loc2_,QueuedCommand.SEEK,param1);
         }
         else
         {
      ...
      flvplayback_internal function createVideoPlayer(param1:Number) : void
      {
         ...
         added = false;
         if(flvplayback_internal::uiMgr.skin_mc != null)
         {
            try
            {
               skinDepth = getChildIndex(flvplayback_internal::uiMgr.skin_mc);
               if(skinDepth > 0)
               {
      ...
      public function set visibleVideoPlayerIndex(param1:uint) : void
      {
         ...
         if(flvplayback_internal::_firstStreamShown)
         {
            flvplayback_internal::uiMgr.setupSkinAutoHide(false);
            _loc2_.visible = true;
            _soundTransform.volume = !scrubbing ? Number(0) : Number(_volume);
            _loc2_.soundTransform = _soundTransform;
         }
         else if((_loc2_.stateResponsive || _loc2_.state == VideoState.CONNECTION_ERROR || _loc2_.state == VideoState.DISCONNECTED) && flvplayback_internal::uiMgr.skinReady)
         {
            ...
            {
               flvplayback_internal::uiMgr.hookUpCustomComponents();
            }
            showFirstStream();
         }
         ...
         flvplayback_internal::uiMgr.handleIVPEvent(new VideoEvent(VideoEvent.STATE_CHANGE,false,false,state,playheadTime,_visibleVP));
         flvplayback_internal::uiMgr.handleIVPEvent(new VideoEvent(VideoEvent.PLAYHEAD_UPDATE,false,false,state,playheadTime,_visibleVP));
         if(_loc2_.isRTMP)
         {
            flvplayback_internal::uiMgr.handleIVPEvent(new VideoEvent(VideoEvent.READY,false,false,state,playheadTime,_visibleVP));
         }
         else
         {
            flvplayback_internal::uiMgr.handleIVPEvent(new VideoProgressEvent(VideoProgressEvent.PROGRESS,false,false,bytesLoaded,bytesTotal,_visibleVP));
         }
         _activeVP = _loc4_;
      }
      ...
      public function play(param1:String = null, param2:Number = NaN, param3:Boolean = false) : void
      {
         ...
         if(param1 == null)
         {
            if(!flvplayback_internal::_firstStreamShown)
            {
               ...
               queueCmd(_loc4_,QueuedCommand.PLAY);
            }
            else
            {
      ...
      public function playWhenEnoughDownloaded() : void
      {
         ...
         {
            _loc1_ = flvplayback_internal::videoPlayerStates[_activeVP];
            queueCmd(_loc1_,QueuedCommand.PLAY_WHEN_ENOUGH);
         }
         else
         {
            _loc2_ = flvplayback_internal::videoPlayers[_activeVP];
      ...
      flvplayback_internal function skinLoaded() : void
      {
         ...
         else
         {
            ...
            skinShowTimer = new Timer(flvplayback_internal::DEFAULT_SKIN_SHOW_TIMER_INTERVAL,1);
