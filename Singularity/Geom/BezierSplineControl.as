package Singularity.Geom
{
   ...
   public class BezierSplineControl
   {
      ...
      private function __right(param1:Number) : void
      {
         ...
         var dX:Number = NaN;
         var dY:Number = NaN;
         var _t:Number = param1;
         try
         {
            if(this.__dist > Consts.ZERO_TOL)
            {
               count = this.__points.length - 1;
               mX = 0.5 * (this.__points[count - 1].X + this.__points[count].X);
               mY = 0.5 * (this.__points[count - 1].Y + this.__points[count].Y);
               pX = this.__points[count].X - mX;
               pY = this.__points[count].Y - mY;
               n = 2 / this.__d2;
               nX = -n * pY;
               nY = n * pX;
               a11 = nX * nX - nY * nY;
            ...
            {
               this.__pX = this.__points[count].X - _t * this.__dX2;
               this.__pY = this.__points[count].Y - _t * this.__dY2;
            }
            coef = this.__cage[count - 1];
            coef.P0X = this.__points[count - 1].X;
            coef.P0Y = this.__points[count - 1].Y;
            coef.P1X = this.__bXNL;
            coef.P1Y = this.__bYNL;
            coef.P2X = this.__pX;
            coef.P2Y = this.__pY;
            coef.P3X = this.__points[count].X;
            coef.P3Y = this.__points[count].Y;
         }
         catch(err:Error)
         {
         }
