package anifire.playback
{
   ...
   public class PlayerConstant
   {
      ...
      public static function getBehaviourXMLfromThemeXML(param1:String, param2:String, param3:UtilHashArray) : XML
      {
         ...
         {
            var _loc6_:int = 0;
            var _loc7_:* = §§checkfilter(themeXML.char);
            var _loc5_:* = new XMLList("");
            §§push(§§findproperty(XMLList));
            themeXML.char.(@id == charID);
            §§pop().§§slot[9] = (§§newactivation()).XMLList(_loc5_)[0];
            if(behaviourNodeName == "action")
            {
               _loc6_ = 0;
               _loc7_ = §§checkfilter(charNode..action);
               _loc5_ = new XMLList("");
               §§push(§§findproperty(XMLList));
               charNode..action.(@id == behaviourID);
               §§pop().§§slot[8] = (§§newactivation()).XMLList(_loc5_)[0];
               if(resultXML == null)
               {
                  _loc6_ = 0;
                  _loc7_ = §§checkfilter(charNode..motion);
                  _loc5_ = new XMLList("");
                  §§push(§§findproperty(XMLList));
                  charNode..motion.(@id == behaviourID);
                  §§pop().§§slot[8] = (§§newactivation()).XMLList(_loc5_)[0];
               }
            }
